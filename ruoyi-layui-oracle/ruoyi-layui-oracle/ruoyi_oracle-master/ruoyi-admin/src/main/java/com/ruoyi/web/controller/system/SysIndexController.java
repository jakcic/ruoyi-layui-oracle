...
public class SysIndexController extends BaseController
{
    ...
    public String index(ModelMap mmap)
    {
        ...
        // 根据userId查询upId
        List<Long> ups = userService.findupIdByUserId(user.getUserId());

        // 根据upId查询postId
        List<Long> posts = new ArrayList<>();
        for (Long up : ups) {
            Long post = postService.findpostIdByupId(up);
            posts.add(post);
        }
        ...
        for (Long postId : postList) {
            List<Long> role = postService.findroleIdBypostId(postId);
            roles.add(role);
        }

        /**
         * 根据角色id查询菜单信息
         * 这一步有问题，如果有多个角色需要怎么查询亟待解决
         */
